...
class MultiHeadVCL(nn.Module):
    def __init__(self, in_dim: int, shared_hidden_dims: List[int], head_hidden_dims: List[int], head_out_dim: int, num_heads=1, initialization: Initialization = Initialization.DEFAULT, **kwargs):
        ...
        if initialization is not Initialization.DEFAULT:
            with torch.no_grad():
                if initialization is Initialization.CUSTOM:
                    assert 'custom_init' in kwargs
                    self.set_params(*kwargs['custom_init'].get_params())
                else:
                    self.init_shared_weights(initialization)
                    self.init_heads(initialization)
    ...
    def new_from_prior(prior_model: 'MultiHeadVCL') -> 'MultiHeadVCL':
        ...
        with torch.no_grad():
            model.set_params(*prior_model.get_params())
    ...
    def init_heads(self, initialization: Initialization):
        with torch.no_grad():
            for head in self.heads:
                if initialization is Initialization.DEFAULT:
                    head.reinit_each_layer()
                    ...
                    head.reinit_each_layer_with(init_layer_small_variance)
                else:
                    raise NotImplementedError(
                        "Weight initialisation with method:", initialization)
    ...
    def predict_MLE(self, x, task_i_mask: Union[int, np.int64, torch.Tensor]):
        ...
        if type(task_i_mask) is int or type(task_i_mask) is np.int64:
            logits = self.heads[task_i_mask].forward_MLE(
                hiddens, [nn.ReLU() for i in range(len(self.heads[task_i_mask].layers) - 1)] + [nn.Identity()])
        else:
            assert type(task_i_mask) is torch.Tensor, type(task_i_mask)
            ...
            for task_i, mask in task_datapoints:
                logits[mask] = self.heads[task_i].forward_MLE(
                    [hidden[mask] for hidden in hiddens], [nn.ReLU() for i in range(len(self.heads[task_i].layers) - 1)] + [nn.Identity()])
        return logits
    ...
    def set_params(self, locs, scales):
        ...
        for key in self.layer_dict:
            if key == 'heads':
                for head in self.layer_dict[key]:
                    _locs, _scales = head.get_params()
                    size = _locs.shape[0]
                    head.set_params(locs[next_i:next_i+size],
                                    scales[next_i:next_i+size])
                    next_i += size
            else:
                _locs, _scales = self.layer_dict[key].get_params()
                size = _locs.shape[0]
...
class MultiHeadVCLSplitMNIST(MultiHeadVCL):
    def __init__(self, num_heads=1, initialization: Initialization = Initialization.DEFAULT, **kwargs):
        ...
        super(MultiHeadVCLSplitMNIST, self).__init__(
            in_dim=28*28, shared_hidden_dims=[256, 256], head_hidden_dims=[], head_out_dim=2, num_heads=num_heads, initialization=initialization, **kwargs)
...
class MultiHeadVCLSplitNotMNIST(MultiHeadVCL):
    def __init__(self, num_heads=1, initialization: Initialization = Initialization.DEFAULT, **kwargs):
        ...
        super(MultiHeadVCLSplitNotMNIST, self).__init__(
            in_dim=28*28, shared_hidden_dims=[150, 150, 150, 150], head_hidden_dims=[], head_out_dim=2, num_heads=num_heads, initialization=initialization, **kwargs)
